...
程序的细节上需要注意以下几点
- 初始值: `right`的初始值是`n`, 表示不包含`n`
- 终止值: `while`的条件是`left<right`, 表示当`left==right`区间无效
- 过程值: `if(nums[mindex]<target)`的处理是`left = mindex+1`表示左闭, `else`处理是`right = mindex`表示右开

注意位运算`>>`的优先级低于`+`, 因此要加括号计算`int mindex = left + ((right-left)>>1);`
```java
...
上述①和②是相邻关系, 并且`<x` 和 `≥x` 互为补集，两侧元素个数之和为 n
...
- 「求最小」和二分查找求「排序数组中某元素的第一个位置」是类似的，逻辑判别中的等号给了指针`r`, 最终答案就是`r`, 最小值左边是不满足要求的，右边则是满足要求的
- 「求最大」, 逻辑判别中的等号需要给指针`l`, 最终答案就是`l`, 最大值左边也是满足要求的，右边是不满足要求的。

**强调, 二分答案法中「求最小」与「求最大」均与传统二分重复元素无关, 我们只是满足题意上逻辑上大小的「表述」, 二分答案序列本身是不可能重复的**

二分答案的计算往往不会是一个简单的条件, 我们可以抽象出复杂的比较函数(猜答案函数)

_「二分答案求最小」模板_
checkDes: 即判断 `nums[mid] >= target` 为true, 逻辑等号留给`right`, 区间降低
```java
public int lowerBound(int[] nums, int target){
    //二分答案开区间写法, while(`x √ x`)
    while(l + 1 < r){
...
//在checkDes函数中进行具体计算, 当计算结果逻辑大于等于target则返回true
boolean checkDes(int[] nums, int mid , int target){
    return computeIncheck(...) 逻辑大于等于 target;
}
```
...
checkDes(int mid, int[] mark, int target){
...
首先是等号放给`l`, 即checkInc为`return computeIncheck(...) 逻辑小于等于 target` 则 `l=mid`, 意味区间增长
