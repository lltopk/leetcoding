 ...
 * 然后用Floyd判断是否存在环
 *
 * 为了降低时间复杂度, 可以用标记快慢指针走过的节点, 下次这些节点就不能作为起点建图了直接跳过
 */
public class LC457_circularArrayLoop2 {
    /**
    ...
    public boolean circularArrayLoop(int[] nums) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            //有可能是上面的while循环出现了异号, 此时slow走的慢, 因此下面只标记slow走过的节点, 下次不能作为起点建图
            int add = i;
            while (mark[add] == 0 && nums[add] * nums[next(nums, add)] > 0) {
                mark[add] = 1;
                add = next(nums, add);
            }
        }
