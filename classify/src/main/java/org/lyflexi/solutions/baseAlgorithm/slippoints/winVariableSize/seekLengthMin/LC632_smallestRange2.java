 ...
 *
 * 优化了LC632_smallestRange, 省去了shrink的O(k)判断, 维护int cnt0 = k;//记录不包含list的总次数, 滑动窗口有效的条件是cnt0 == 0
 *
 * 优化后时间复杂度前面是二维数组初始化, 中间是合并后的数组排序, 后面是滑动窗口(从total*k降低为total)
 */
public class LC632_smallestRange2 {
    //不定长滑动窗口求最小
    public int[] smallestRange(List<List<Integer>> nums) {
        ...
        int cnt0 = k;//记录不包含list的总次数, 滑动窗口有效的条件是cnt0 == 0

        //对pairs进行不定长滑动窗口
        for(int r = 0; r < total; r++){
            helper[pairs[r][1]]++;
            //首次出现某个list, cnt0--, 需要注意不能重复--
            ...
            while(cnt0 == 0){
                ...
                //同理当最后一个list即将移出窗口, cnt0++, 需要注意不能重复++
