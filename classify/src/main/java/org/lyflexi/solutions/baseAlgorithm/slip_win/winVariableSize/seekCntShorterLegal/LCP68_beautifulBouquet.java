package org.lyflexi.solutions.baseAlgorithm.slip_win.winVariableSize.seekCntShorterLegal;
...
public class LCP68_beautifulBouquet {
    public int beautifulBouquet(int[] flowers, int cnt) {
        //不定长滑动窗口求个数, 越短越合法
        int n = flowers.length, ans = 0, l = 0;
        Map<Integer, Integer> dict = new HashMap<>();
        for(int r = 0; r< n; r++){
            dict.merge(flowers[r], 1, Integer::sum);

            while(dict.get(flowers[r]) >cnt){

                if(dict.get(flowers[l]) == 1){
                    dict.remove(flowers[l]);
                }else{
                    dict.merge(flowers[l], -1, Integer::sum);
                }
                l++;
            }
