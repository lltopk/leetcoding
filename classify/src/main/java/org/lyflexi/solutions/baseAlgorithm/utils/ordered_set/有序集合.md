...
## SortedList
`python`自带了有序集合`SortedList`, 实现复杂, 它是基于数组位置的线性顺序, 因此往往用来计算`rank`, 但很遗憾`Java`没有

`Java`的`TreeMap`或者`TreeSet`底层是红黑树, 它的有序性只是体现在中序遍历上, 因此红黑树只知道目标节点的左右大小关系, 无法对整体数组进行`rank`