...
正向遍历的痛点就是无法去获取未来的元素，只要将未解决的元素(索引)用栈存储起来，等后面满足条件再一一匹配

维护单调栈, 可以正向遍历, 也可以反向遍历, 但栈中存储的都是元素下标. 

时间复杂度分析: 摊还分析, 虽然是个二重循环, 但是最内层循环中的`st.pop()`顶多执行`N`次, 因此是`O(N)`

### 正向遍历维护单调栈
通过维护单调栈顶, 计算 `left[]`，其中 `left[i]` 是 `nums[i]` 左侧最近的严格大于 `nums[i]` 的数的下标，若不存在则为 `-1`
```java
class Solution {
    // 返回 left，其中 left[i] 是 nums[i] 左侧最近的严格大于 nums[i] 的数的下标，若不存在则为 -1
    // 时间复杂度 O(nums.length)
    private int[] leftGreater(int[] nums) {
        int n = nums.length;
        int[] left = new int[n];
...
### 反向遍历维护单调栈
通过维护单调栈顶, 计算 `right[]`，其中 `right[i]` 是 `nums[i]` 右侧最近的严格大于 `nums[i]` 的数的下标，若不存在则为 `nums.length`
```java
class Solution {
    // 返回 right，其中 right[i] 是 nums[i] 右侧最近的严格大于 nums[i] 的数的下标，若不存在则为 nums.length
    // 时间复杂度 O(nums.length)
    private int[] rightGreater(int[] nums) {
        int n = nums.length;
        int[] right = new int[n];
        Deque<Integer> st = new ArrayDeque<>();
        st.push(n); // 哨兵
