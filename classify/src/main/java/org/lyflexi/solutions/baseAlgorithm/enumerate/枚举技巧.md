...
前后缀两个辅助结构: 
- `pre`简单情况下是个值, 复杂情况下有可能是个辅助数组`pre[]`, 往往可以滚动计算, 无需预初始化
...
## 枚举对角线
### 正向对角线`/`枚举
正向对角线的规律是任意格子`i+j`都是个定值, 最中间的对角线`i+j = m-1`, 意味着这是第`正向对角线总数/2 + 1`条正向对角线
...
最终k取值范围`[0, m+n-2]`, 这恰好是正向对角线个数, 然后就可以枚举对角线了
...
### 反向对角线`\`枚举
反向对角线的规律是任意格子`i-j`都是个定值, 比如最中间的对角线`i-j = 0`, 此时代表这是第`反向对角线总数/2+1`条对角线
...
k取值范围`[1, m-1+n]`, 这恰好是反向对角线个数, 然后就可以枚举对角线了
