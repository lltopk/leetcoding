...
这时候二分计算往往不会是一个简单的条件, 我们可以抽象出复杂的比较函数(猜答案函数), 依然以lowerBound算法为例
...
~~如果你习惯于拿`checkDes`函数来缩减下标, 那么同理`return computeIncheck(...) >= target` 则 `r = mid` 反之 `l=mid+1`. 只是注意`checkDes`函数逻辑正好与`checkInc`函数逻辑相反~~
