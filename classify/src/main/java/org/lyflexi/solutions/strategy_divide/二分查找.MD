...
~~同理你还可以写`checkInc return computeIncheck(...) >= target` 则 `r = mid` 反之 `l=mid+1`~~, 

但个人习惯写`compareInc return computeIncheck(...) < target` 则 `l=mid+1` 反之 `r = mid`, 这类题目有下面几个
- LC2300(咒语和药水的成功对数)
- LC33(搜索旋转排序数组)
- LC274(H指数)
- LC275(H指数Ⅱ)
...
另外有些更为复杂的问题会在check的过程中不得不"修改"原数组, 为了避免影响其他二分点的场景分布情况, 我们需要借助**辅助数组int[] mark/last**来标记原数组

1. 每次check过程中初始化辅助数组`Arrays.fill(mark, -1)`, 避免干扰其他二分点的状态分布
3. 对于原数组是字符串的情况, 最好也是借助辅助数组, 因为字符串API性能低下且有记忆负担
...
**上述我们习惯于比较`右端点`或者`nums[mid+1]`**, ~~如果你习惯于比较`左端点`或者`nums[mid]比较nums[mid-1]`也可以, 那么同理在二分过程中会取到`nums[-1]`越界, 此时注意必须初始化l,r指针`[1,n)`~~
