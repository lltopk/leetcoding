...
个人习惯, 上面二者compareInc~~和compareDesc~~选其一即可, 这类题目有下面几个
- LC2300(咒语和药水的成功对数)
- LC33(搜索旋转排序数组)
- LC274(H指数)
- LC275(H指数Ⅱ)
...
**另外有些问题会在check的过程中不断"修改"原数组, 为了避免影响其他二分点的场景分布情况, 我们需要借助辅助数组(int[] mark)来标记原数组**

对于原数组是字符串的情况, 最好也是借助辅助数组(int[] mark), 因为字符串API性能低下且有记忆负担
...
```
或者直接以以二分值mid为标记值, 这样不用每次都check都要清空mark分布, 因为各个check的mark分布都是独立的

但要提前初始化标记数组为-1, 避免与二分点0干扰
```java
int [] mark = new int[len];
for(int i =0;i<len;i++){
    mark[i] = -1;
