...
## 左闭右开
特点: l左侧始终是小于目标值的元素, r右侧始终是大于目标值的元素, 因此最终l,r交会处就是解
...
1. compareInc: 即判断 `nums[mid] < target` 为true.
**循环不变量:** 在初始化（循环之前）、循环中、循环结束后，都时时刻刻保证 `check(left) == true && check(right-1) == false`
```java
public int lowerBound(int[] nums, int target){
    while(l<r){
        int mid = (l+r)>>1;
...
2. compareDesc: 即判断 `nums[mid] >= target` 为true.
**循环不变量:** 在初始化（循环之前）、循环中、循环结束后，都时时刻刻保证 `check(right-1) == true && check(left) == false`
```java
public int lowerBound(int[] nums, int target){
    while(l<r){
        int mid = (l+r)>>1;
