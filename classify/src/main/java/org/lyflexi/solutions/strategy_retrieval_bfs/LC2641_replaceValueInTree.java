 ...
 * 76.8%
 */
public class LC2641_replaceValueInTree {
    ...
    public TreeNode replaceValueInTree(TreeNode root) {
        ...
        deque.offer(root);
        while(! deque.isEmpty()){
            int nextLayerSum = 0, n = deque.size();
            List<TreeNode> bakLayer = new ArrayList<>(deque);
            ...
            for(TreeNode node: bakLayer){
                int childrenSum = (node.left == null? 0: node.left.val) + (node.right == null? 0: node.right.val);
                if(node.left != null) node.left.val = nextLayerSum - childrenSum;
                if(node.right != null) node.right.val = nextLayerSum - childrenSum;
            }
        }
        return root;
    }
