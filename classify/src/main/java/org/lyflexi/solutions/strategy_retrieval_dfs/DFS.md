...
那么对于所有自然数`n`, `P(n)`都成立。基于数学归纳法可以证明递归结果必定正确，无论是自底向上`DFS`或是自顶向下`DFS`, 目标最终都是`f(n)`。

由于递归在相邻层级间有强关联，落实到具体代码上，需要抽象出大`F`代表具体的计算逻辑，最终我们要模拟的计算公式为`f(i+1) = F(f(i))`
- 如果是自底向上`DFS`, `n`是顶, 那么`f(i)`就是递归函数本身`dfs`, `f(i+1) = F(f(i))`则是基于子问题结果, 来计算上层问题
- 如果是自顶向下`DFS`, `n`是底, 那么`f(i)`就是递归过程中的状态变量`state/layer`, `f(i+1) = F(f(i))`代表的是不同层之间状态变量的递进过程
...
public int dfs(...args){
    ...
    //自底向上递归模板, 先递归后计算
    option(ret)//可选操作， 自底向上模板大部分情况函数的返回值就是最终答案了， 但不排除特殊的题目还需要全局ret来接收（可选）, 如145. 二叉树的后序遍历, 1026. 节点与其祖先之间的最大差值, 563. 二叉树的坡度
...
既然归都是存在于递归函数的, 那么自顶向下也可以借鉴自底向上的整体思维带上返回值, 但必须是当前问题(父节点)依赖于子问题的结果进行计算/汇总

**这类题目的特征是既有自顶向下的`state/layers`向下传递计算, 又有当前问题(父节点)依赖于子问题的结果进行计算/汇总**

我把这类题叫做: 自顶向下有递也有归, 见LC129. 求根节点到叶节点数字之和: 
```java
...
```
更多自顶向下有递也有归的题目见LC112. 路径总和解法二; 226. 翻转二叉树解法二; 1448. 统计二叉树中好节点的数目解法二; 1022. 从根到叶的二进制数之和解法二

最后注意需要区分的是 自顶向下有递有归 与 单独的自底向上归, 比如LC100. 相同的树, 只是单纯的利用当前节点值`root.val`做计算, 由于不存在`state/layers`参数向下传递计算, 因此本质上仍然属于自底向上归的
```java
