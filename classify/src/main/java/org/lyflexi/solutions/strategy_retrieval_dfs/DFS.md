...
## 数学归纳法
使用DFS/递归来解决问题往往是存在相同子问题, 如何证明子问题->目标问题, 请运用初高中知识, 数学归纳法:
...
那么对于所有自然数`n`, `P(n)`都成立。具体在递归代码执行层面, **还需加上递归出口**, 以计算二叉树深度为例LC104. 二叉树的最大深度

一. 自底向上归
- 归纳奠基 `P(1)`成立: 对应于递归出口, 当前节点为`null`, 注意`null`代表当前节点连叶子节点都不是, 直接返回高度`0`
- 归纳步骤: 递归调用子问题, 当前节点不为`null`, 则其高度为子问题的高度`+1`

二. 自顶向下递
- 归纳奠基: 定义新的递归函数`dfs(root, ...states)`, 并给新的递归函数`dfs`传入初始值`0`
- 归纳步骤: 先累加高度, 然后递归调用子问题, 当前节点不为`null`, 则其高度为当前`depth+1`

由于递归程序会用栈隐式的记忆每个节点的信息, 来模拟节点后进先出归的效果, 因此空间复杂度是`O(N)`
