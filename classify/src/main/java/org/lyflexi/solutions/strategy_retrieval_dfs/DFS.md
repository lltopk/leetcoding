DFS深度搜索分为「自底向上」和「自顶向下」都是从浅到深DFS, 只不过命名方式关注的是深度信息如何在父子之间传递

如何不晕理解递归, 以计算二叉树深度为例, 可以把递归与循环求和数组元素进行对比, **都需要定义初始值, 以及循环执行相同代码**

一. 自底向上归
- 归的初始值: 对应于递归出口, 当前节点为`null`, 注意`null`代表当前节点连叶子节点都不是, 直接返回高度`0`
- 循环执行相同代码: 递归调用子问题, 当前节点不为`null`, 则其高度为子问题的高度`+1`

二. 自顶向下递
- 递的初始值: 定义新的递归函数`dfs`, 并给新的递归函数`dfs`传入初始值`0`
- 循环执行相同代码: 递归调用子问题, 当前节点不为`null`, 则其高度为当前`depth+1`

递归程序会用栈隐式的记忆每个节点的信息, 来实现后进先出, 因此空间复杂度是`O(N)`
